...
class ezpzPostType {
  ...
  public function __construct($post_type_names, $icon = null, $options = array()) {
    ...
    if (is_array($post_type_names)) {
      if (!isset($post_type_names['singular'])) {
        throw new Exception(__('You registered an instance of a new CustomPostType but did not pass through a singular. This is required', 'ezpz-posts'));
      } else {
  ...
  function register_post_type() {
    ...
    $labels = array(
      'name'                     => sprintf(__('%s', 'ezpz-posts'), $plural),
      'singular_name'            => sprintf(__('%s', 'ezpz-posts'), $singular),
      'add_new'                  => __('Add New', 'ezpz-posts'),
      'add_new_item'             => sprintf(__('Add New %s', 'ezpz-posts'), $singular),
      'edit_item'                => sprintf(__('Edit %s', 'ezpz-posts'), $singular),
      'new_item'                 => sprintf(__('New %s', 'ezpz-posts'), $singular),
      'view_item'                => sprintf(__('View %s', 'ezpz-posts'), $singular),
      'view_items'               => sprintf(__('View %s', 'ezpz-posts'), $plural),
      'search_items'             => sprintf(__('Search %s', 'ezpz-posts'), $plural),
      'not_found'                => sprintf(__('No %s found', 'ezpz-posts'), $plural),
      'not_found_in_trash'       => sprintf(__('No %s found in trask', 'ezpz-posts'), $plural),
      'parent_item_colon'        => sprintf(__('Parent %s', 'ezpz-posts'), $singular),
      'all_items'                => sprintf(__('All %s', 'ezpz-posts'), $plural),
      'archives'                 => sprintf(__('%s Archives', 'ezpz-posts'), $singular),
      'attributes'               => sprintf(__('%s Attributes', 'ezpz-posts'), $singular),
      'uploaded_to_this_item'    => sprintf(__('Uploaded to this %s', 'ezpz-posts'), $singular),
      'filter_items_list'        => sprintf(__('Filter %s', 'ezpz-posts'), $plural),
      'items_list_navigation'    => sprintf(__('%s list navigation', 'ezpz-posts'), $plural),
      'items_list'               => sprintf(__('%s list', 'ezpz-posts'), $plural),
      'item_published'           => sprintf(__('%s published', 'ezpz-posts'), $singular),
      'item_published_privately' => sprintf(__('%s published privately', 'ezpz-posts'), $singular),
      'item_reverted_to_draft'   => sprintf(__('%s reverted to draft', 'ezpz-posts'), $singular),
      'item_scheduled'           => sprintf(__('%s scheduled', 'ezpz-posts'), $singular),
      'item_updated'             => sprintf(__('%s updated', 'ezpz-posts'), $singular),
      'item_link'                => sprintf(__('%s Link', 'ezpz-posts'), $singular),
      'item_link_description'    => sprintf(__('A link to a %s', 'ezpz-posts'), $singular),
    );
  ...
  public function add_admin_message($message) {
    ...
    add_action('admin_notices', function () use ($message) {
      ...
      if (($pagenow == 'edit.php') && ($_GET['post_type'] == $this->slug)) {
        echo '<div class="notice custom-notice notice-info"><p>' . __($message, 'ezpz-posts') . '</p></div>';
      }
    });
  }
  ...
  function register_taxonomy($taxonomy_names, $options = array()) {
    ...
    if (is_array($taxonomy_names)) {
      if (!isset($taxonomy_names['singular'])) {
        throw new Exception(__('You registered an instance of a new taxonomy but did not pass through a singular. This is required', 'ezpz-posts'));
      } else {
    ...
    if (!taxonomy_exists($slug)) {
      ...
      $labels = array(
        'name'                       => sprintf(__('%s', 'ezpz-posts'), $plural),
        'singular_name'              => sprintf(__('%s', 'ezpz-posts'), $singular),
        'menu_name'                  => sprintf(__('%s', 'ezpz-posts'), $plural),
        'all_items'                  => sprintf(__('All %s', 'ezpz-posts'), $plural),
        'edit_item'                  => sprintf(__('Edit %s', 'ezpz-posts'), $singular),
        'view_item'                  => sprintf(__('View %s', 'ezpz-posts'), $singular),
        'update_item'                => sprintf(__('Update %s', 'ezpz-posts'), $singular),
        'add_new_item'               => sprintf(__('Add New %s', 'ezpz-posts'), $singular),
        'new_item_name'              => sprintf(__('New %s Name', 'ezpz-posts'), $singular),
        'parent_item'                => sprintf(__('Parent %s', 'ezpz-posts'), $plural),
        'parent_item_colon'          => sprintf(__('Parent %s:', 'ezpz-posts'), $plural),
        'search_items'               => sprintf(__('Search %s', 'ezpz-posts'), $plural),
        'popular_items'              => sprintf(__('Popular %s', 'ezpz-posts'), $plural),
        'separate_items_with_commas' => sprintf(__('Seperate %s with commas', 'ezpz-posts'), $plural),
        'add_or_remove_items'        => sprintf(__('Add or remove %s', 'ezpz-posts'), $plural),
        'choose_from_most_used'      => sprintf(__('Choose from most used %s', 'ezpz-posts'), $plural),
        'not_found'                  => sprintf(__('No %s found', 'ezpz-posts'), $plural),
      );
          ...
          if (is_array($this->taxonomies)) {
            ...
            foreach ($this->taxonomies as $tax) {
              ...
              if ($tax !== 'category' && $tax !== 'post_tag') {
                ...
                $new_columns[$tax] = sprintf(__('%s', 'ezpz-posts'), $taxonomy_object->labels->name);
              }
            }
          }
        }
    ...
    switch ($column) {
      ...
      case (taxonomy_exists($column)):
        ...
        if (!empty($terms)) {
          ...
          // Echo no terms.
          printf(__('No %s', 'ezpz-posts'), $taxonomy_object->labels->name);
        }
      ...
      if (!empty($filters)) {
        ...
        foreach ($filters as $tax_slug) {
          ...
          if ($terms) {
            ...
            // Default show all.
            printf('<option value="0">%s</option>', sprintf(__('Show all %s', 'ezpz-posts'), $tax->label));
  ...
  function updated_messages($messages) {
    ...
    $messages[$this->slug] = array(
      0 => '',
      1 => sprintf(__('%s updated.', 'ezpz-posts'), $singular),
      2 => __('Custom field updated.', 'ezpz-posts'),
      3 => __('Custom field deleted.', 'ezpz-posts'),
      4 => sprintf(__('%s updated.', 'ezpz-posts'), $singular),
      5 => isset($_GET['revision']) ? sprintf(__('%2$s restored to revision from %1$s', 'ezpz-posts'), wp_post_revision_title((int) $_GET['revision'], false), $singular) : false,
      6 => sprintf(__('%s updated.', 'ezpz-posts'), $singular),
      7 => sprintf(__('%s saved.', 'ezpz-posts'), $singular),
      8 => sprintf(__('%s submitted.', 'ezpz-posts'), $singular),
      9 => sprintf(
        __('%2$s scheduled for: <strong>%1$s</strong>.', 'ezpz-posts'),
        date_i18n(__('M j, Y @ G:i', 'ezpz-posts'), strtotime($post->post_date)),
        $singular
      ),
      10 => sprintf(__('%s draft updated.', 'ezpz-posts'), $singular),
    );
