...
if ($prefix = wp_strip_all_tags($statistic['statistic_prefix'])) {
  $data_attribs .= ' data-prefix="' . $prefix . '"';
  $initial_value .= '<span class="small">' . $prefix . '</span>';
}

$initial_value .= wp_strip_all_tags($statistic['statistic_value']);

if ($suffix = wp_strip_all_tags($statistic['statistic_suffix'])) {
...
<div class="card card-statistic no-border">
  <div class="card-section">
    ...
    <h3><?php echo wp_strip_all_tags($statistic_title); ?></h3>
    <?php if ($statistic_description) {
      echo '<div class="hide-below-md">' . wp_strip_all_tags($statistic_description) . '</div>';
    } ?>
  </div>
