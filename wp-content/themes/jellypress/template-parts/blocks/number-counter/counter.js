(function ($) {
  ...
  function initialiseCountUp() {
    ...
    if (!!window.IntersectionObserver) {
      ...
      ); // Wait until a slither of the number is visible
      document
        .querySelectorAll(".count-to")
        .forEach((number) => countUpObserver.observe(number));
    }
  }
  ...
  function countUp(number) {
    ...
    if (number.hasAttribute("data-prefix"))
      prefix = '<span class="small">' + $this.attr("data-prefix") + "</span>";
    if (number.hasAttribute("data-suffix"))
      suffix = '<span class="small">' + $this.attr("data-suffix") + "</span>";
    if (number.hasAttribute("data-duration"))
      countDuration = Number($this.attr("data-duration"));
  ...
  if (window.acf) {
    window.acf.addAction(
      "render_block_preview/type=ezpz/number-counter",
      function () {
        initialiseCountUp();
      }
    );
